...
export default function ActiveWorkoutPage({
		...
		async function loadData() {
			try {
				...
				setExerciseGroups(() => {
					...
					const orderedSets = [...nextRepSets, ...nextDurationSets];
	...
	const saveEditedWorkout = async (
		...
		const currentRepSets = exerciseGroups.flatMap((group) =>
			group.sets.filter((set): set is RepSet => "reps" in set),
		);
		const currentDurationSets = exerciseGroups.flatMap((group) =>
			group.sets.filter((set): set is DurationSet => "duration" in set),
		);
													...
													return (
														...
														>
															...
															<Input
																type="number"
																placeholder={
																	actualValues.rpe || set.rpe?.toString() || "0"
																}
																value={
																	inputs[`${set.id}-rpe`] ??
